...
var RegisterComponentManager = /** @class */ (function (_super) {
    tslib_1.__extends(RegisterComponentManager, _super);
    ...
    RegisterComponentManager.prototype.changeStatus = function (key, status) {
        ...
        if (index === -1)
            return;
        this.modifyEntry(tslib_1.__assign(tslib_1.__assign({}, this.state.components[index]), { status: status }), index);
    };
    RegisterComponentManager.prototype.modifyEntry = function (entry, index) {
        var newState = tslib_1.__spreadArray([], this.state.components, true);
