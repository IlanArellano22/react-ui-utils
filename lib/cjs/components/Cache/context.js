...
function setCacheEntry(cache, config, entry) {
    ...
    return {
        entries: tslib_1.__spreadArray([entry], cache.entries.slice(0, config.maxSize - 1), true)
    };
}
...
function CacheFuncReducer(cache, action) {
    switch (action.type) {
        ...
        case "resolvePromise":
            return setExistingCacheEntryById(cache, action.payload.id, function (x) { return (tslib_1.__assign(tslib_1.__assign({}, x), { value: tslib_1.__assign(tslib_1.__assign({}, x.value), { payload: action.payload.value }) })); });
        default:
            return cache;
    }
}
...
var cacheReducer = function (cache, action) {
    ...
    switch (action.type) {
        case "clear":
            return {};
        case "func":
            return tslib_1.__assign(tslib_1.__assign({}, cache), (_a = {}, _a[action.payload.func] = CacheFuncReducer((cache[action.payload.func] || {}), action.payload.action), _a));
    }
};
...
var reducer = function (state, action) {
    ...
    switch (action.type) {
        ...
        case "clearRec":
            return state;
        case "resource":
            return tslib_1.__assign(tslib_1.__assign({}, state), (_a = {}, _a[action.payload.resource] = {
                cache: {},
